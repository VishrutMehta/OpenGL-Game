...
int camera=0, factor =0, win=0, w, h, CAM=5.0, hel_cam_angle=90, tree_x[TREE], tree_z[TREE], cam_row=0, cam_col=0;
float YSC=1.5, i1 = 0.8, i2=0.8, i3=0.8, tim=0;
...
float hel_up_x=0, hel_up_y=0, hel_up_z=1;
...
class Guns
{
	...
	public:
		void drawGuns(float xcoord, float zcoord)
		{
			...
			glPopMatrix();
		/*	glPushMatrix();
			glTranslatef(bullet_x, -1.5, zcoord);
			gluSphere(qgun, 0.3, 50,50);
			glPopMatrix();*/
		}
}gun[GUNS];
class Tree
...
void sound()
{
	...
	if(x==0)
	{
		execlp("mpg123", "mpg123", "-q", "./beep.mp3", 0); 
	}
	else
	{
		system(kil);
...
void update(int value) {
	...
	switch(camera)
	{
		case 0: gluLookAt( 0, 15, 35, 0,-5,-10, 0,5,0); break;
...
void handleKeypress1(unsigned char key, int x, int y) {
    ...
    if(camera == 3)
    {
	    if(key == 's')
	    {
		    hel_eye_x = hel_eye_x + LAMBDA*cos(hel_cam_angle*PI/180);
		    hel_eye_z = hel_eye_z - LAMBDA*sin(hel_cam_angle*PI/180);
	//	    hel_eye_z-=CAMERA;
            }
	    if(key == 'a')
	    {
		    if(hel_cam_angle < 180)
		  	  hel_cam_angle+=2;
		    else
			  hel_cam_angle=hel_cam_angle-360;
		    printf("angle=%d\n", hel_cam_angle);
	//	    hel_eye_x-=CAMERA;
            }
	    if(key == 'w')
